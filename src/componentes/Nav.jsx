  ...
  return (
    <nav className='flex justify-center m-auto'>
      <div className={`menuHamburguer bg-naranja ${menu ? 'translate-x-[0] ease-out duration-300 justify-center items-center' : 'translate-x-[500px] ease-out duration-300'} sm:translate-x-[0] m-0 rounded-none flex flex-col fixed top-0 w-[100vw] h-[100vh] z-10 sm:w-auto sm:h-auto sm:relative sm:flex sm:flex-row gap-10 px-10 py-3 sm:rounded-2xl bg-black sm:my-14`}>
        <Link className="text-white text-[10px] smd:text-[15px] cursor-pointer" to="inicio" duration={500} onClick={cerrarMenu}>
          INICIO
        </Link>
        <Link className="text-white text-[10px] smd:text-[15px] cursor-pointer" to="proyectos" duration={500} offset={-300} onClick={cerrarMenu}>
          PROYECTOS
        </Link>
        <Link className="text-white text-[10px] smd:text-[15px] cursor-pointer" to="habilidades" duration={500} offset={-200} onClick={cerrarMenu}>
          HABILIDADES
        </Link>
        <Link className="text-white text-[10px] smd:text-[15px] cursor-pointer" to="contacto" duration={500} onClick={cerrarMenu}>
          CONTACTAME
        </Link>
      </div>
